    ...
    <div class="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 pb-24 transition-colors duration-500">
      ...
      <div class="mb-8 space-y-4">
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight leading-none">Lá»ch sá»­</h1>
            <p class="text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-2">Theo dÃµi chi tiÃªu cá»§a báº¡n</p>
          </div>
        ...
        <div class="space-y-4">
          ...
          <div *ngIf="showFilters()" class="bg-white dark:bg-slate-900 rounded-[32px] p-6 space-y-6 border border-slate-100 dark:border-slate-800 shadow-xl">
            <div class="flex items-center justify-between">
              <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Bá» lá»c nÃ¢ng cao</h3>
              <button (click)="clearFilters()" class="text-[9px] font-black text-rose-500 uppercase tracking-widest">XÃ³a háº¿t</button>
            </div>

            <div class="grid grid-cols-2 gap-4">
              <div class="col-span-2">
                <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-2">Loáº¡i giao dá»ch</label>
                <div class="flex p-1 bg-slate-50 dark:bg-slate-800 rounded-2xl">
                  <button (click)="filterType.set('all')" [class]="filterType() === 'all' ? 'bg-white dark:bg-slate-900 shadow-sm text-emerald-600' : 'text-slate-400'" class="flex-1 py-2 text-[9px] font-black uppercase rounded-xl transition-all">Táº¥t cáº£</button>
                  <button (click)="filterType.set('income')" [class]="filterType() === 'income' ? 'bg-white dark:bg-slate-900 shadow-sm text-emerald-600' : 'text-slate-400'" class="flex-1 py-2 text-[9px] font-black uppercase rounded-xl transition-all">Thu nháº­p</button>
                  <button (click)="filterType.set('expense')" [class]="filterType() === 'expense' ? 'bg-white dark:bg-slate-900 shadow-sm text-rose-600' : 'text-slate-400'" class="flex-1 py-2 text-[9px] font-black uppercase rounded-xl transition-all">Chi tiÃªu</button>
                </div>
              </div>
              <div>
                <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-2">Tá»« ngÃ y</label>
              ...
              <div>
                <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-2">Äáº¿n ngÃ y</label>
      ...
      <div class="space-y-10 relative">
        ...
        <div *ngIf="isLoading()" class="absolute inset-x-0 -top-4 flex justify-center z-10">
          <div class="bg-white dark:bg-slate-900 shadow-xl border border-slate-100 dark:border-slate-800 px-4 py-2 rounded-full flex items-center gap-2 animate-bounce">
            ...
            <span class="text-[9px] font-black uppercase tracking-widest text-slate-500">Äang táº£i...</span>
          </div>
        </div>
        ...
        <div *ngFor="let group of groupedTransactions()">
          <div class="flex items-center justify-between mb-4 px-2">
            ...
            <span class="text-[10px] font-black text-slate-900 dark:text-slate-100 bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full">{{ group.total | number:'1.0-0' }}â«</span>
          </div>
          ...
          <div class="space-y-3">
            <div *ngFor="let t of group.transactions" class="bg-white dark:bg-slate-900 p-5 rounded-[28px] shadow-sm border border-slate-100 dark:border-slate-800 group hover:shadow-md transition-all duration-300">
              <div class="flex items-center gap-4">
                ...
                <div class="flex-1">
                  <div class="flex items-center justify-between">
                    <h3 class="font-black text-slate-900 dark:text-white text-sm tracking-tight capitalize">{{ t.note || t.category?.name || 'Giao dá»ch' }}</h3>
                    <div class="flex flex-col items-end">
                      <span [class]="t.type === 'income' ? 'text-emerald-500' : 'text-rose-500'" class="font-black text-base tracking-tight">
                        {{ t.type === 'income' ? '+' : '-' }}{{ t.amount | number:'1.0-0' }}â«
                      </span>
                    </div>
                  </div>
                  <div class="flex items-center justify-between mt-1">
                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ t.category?.name || 'KhÃ¡c' }}</span>
                    <div class="flex gap-2">
        ...
        <div *ngIf="transactionService.hasMoreHistory() && !isLoading()" class="pt-4 pb-8 flex justify-center">
           <button (click)="onLoadMore()" class="group relative px-8 py-3 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-sm hover:shadow-md transition-all">
             <span class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 group-hover:text-emerald-500 transition-colors">Xem thÃªm giao dá»ch</span>
           </button>
        </div>
        ...
        <div *ngIf="groupedTransactions().length === 0 && !isLoading()" class="pt-20 text-center">
          ...
          <h3 class="text-lg font-black text-slate-900 dark:text-white tracking-tight">KhÃ´ng tÃ¬m tháº¥y giao dá»ch</h3>
          <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-2">Thá            <!-- Edit Modal (Full Screen on Mobile) -->
      <div *ngIf="isEditModalOpen()" class="fixed inset-0 z-[100] flex flex-col items-center justify-end sm:justify-center animate-in fade-in duration-300">
        <!-- Backdrop -->
        <div class="absolute inset-0 bg-slate-950/60 backdrop-blur-sm" (click)="isEditModalOpen.set(false)"></div>
        
        <!-- Modal Content Container -->
        <div class="relative w-full h-full sm:h-auto sm:max-w-lg bg-white dark:bg-slate-900 rounded-t-[40px] sm:rounded-[40px] shadow-2xl flex flex-col animate-in slide-in-from-bottom-full duration-500 overflow-hidden">
            ...
            <form *ngIf="editingTransaction()" class="space-y-6">
              <div class="space-y-2 text-center pb-4">
                ...
                <div class="flex p-1.5 bg-slate-50 dark:bg-slate-800 rounded-[24px] max-w-[280px] mx-auto border border-slate-100 dark:border-slate-700/50">
                  <button type="button" 
                    (click)="editingTransaction.update(t => t ? ({...t, type: 'expense'}) : null); resetCategoryIfMismatch()"
                  ...
                  <button type="button"
                    (click)="editingTransaction.update(t => t ? ({...t, type: 'income'}) : null); resetCategoryIfMismatch()"
              ...
              <div class="space-y-2">
                ...
                <input type="text" 
                  [ngModel]="formatAmount(editingTransaction()!.amount)" 
                  (ngModelChange)="onAmountChange($event)" 
                  name="amount" 
                  class="w-full px-6 py-5 bg-slate-50 dark:bg-slate-800 border-none rounded-[24px] text-3xl font-black text-slate-900 dark:text-white outline-none focus:ring-4 focus:ring-emerald-500/10 transition-all text-center"
                  placeholder="0">
              </div>
                ...
                <div class="grid grid-cols-4 gap-3 max-h-48 overflow-y-auto pr-2 no-scrollbar">
                  <button type="button"
                    *ngFor="let cat of filteredCategories()"
                    ...
                    [class]="editingTransaction()?.categoryId === cat.id ? 
                      (editingTransaction()?.type === 'expense' ? 'bg-rose-500 text-white shadow-lg shadow-rose-500/20' : 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20') + ' scale-105 ring-4' : 
...
export class TransactionHistoryComponent {
  ...
  constructor() {
    // Tá»± Äá»ng táº£i láº¡i lá»ch sá»­ khi cÃ³ báº¥t ká»³ bá» lá»c nÃ o thay Äá»i
    effect(() => {
      const filters = {
    ...
    // KhÃ³a scroll cá»§a body khi má» Modal
    effect(() => {
  ...
  async onDelete(id: string) {
    if (confirm('Báº¡n cÃ³ cháº¯c cháº¯n muá»n xÃ³a giao dá»ch nÃ y khÃ´ng?')) {
      try {
        await this.transactionService.deleteTransaction(id as string);
        // Táº£i láº¡i lá»ch sá»­ sau khi xÃ³a
        this.onLoadMore(); // Hoáº·c reset vá» trang 1
      } catch (error) {
        alert('Lá»i khi xÃ³a giao dá»ch. Vui lÃ²ng thá»­ láº¡i!');
      }
    }
  }
  ...
  async onSaveEdit() {
    ...
    if (t) {
      try {
        ...
        // Táº£i láº¡i lá»ch sá»­ sau khi sá»­a
      } catch (error) {
        alert('Lá»i khi cáº­p nháº­t giao dá»ch. Vui lÃ²ng kiá»m tra láº¡i!');
      }
    }
  }
  ...
  groupedTransactions = computed(() => {
    // Dá»¯ liá»u á» ÄÃ¢y ÄÃ£ ÄÆ°á»£c Backend lá»c sáºµn
  ...
  private formatDateLabel(dateStr: string): string {
    ...
    const days = ['Chá»§ nháº­t', 'Thá»© hai', 'Thá»© ba', 'Thá»© tÆ°', 'Thá»© nÄm', 'Thá»© sÃ¡u', 'Thá»© báº£y'];
  ...
  onAmountChange(value: string) {
    ...
    if (this.editingTransaction()) {
      this.editingTransaction.update(t => t ? ({...t, amount: Number(rawValue)}) : null);
    }
  }

  resetCategoryIfMismatch() {
    const t = this.editingTransaction();
    if (t) {
      const cats = this.categoryService.categories().filter(c => c.type === t.type);
      if (cats.length > 0 && !cats.find(c => c.id === t.categoryId)) {
        this.editingTransaction.update(prev => prev ? ({...prev, categoryId: cats[0].id}) : null);
      }
    }
  }
}
